...
describe('converterSection', () => {
  ...
  it('should convert text to NATO alphabet', async () => {
    ...
    const conversionSection = screen
      .getByText('NATO Alphabet')
      .closest('.rounded-lg');
    expect(conversionSection).toBeInTheDocument();

    // Use getAllByText and verify we have at least one of each in badges
    expect(
      screen.getAllByText('Alpha').some((el) => el.closest('.bg-blue-100'))
    ).toBe(true);
    expect(
      screen.getAllByText('Bravo').some((el) => el.closest('.bg-blue-100'))
    ).toBe(true);
    expect(
      screen.getAllByText('Charlie').some((el) => el.closest('.bg-blue-100'))
    ).toBe(true);
  });
  ...
  it('should handle numbers and special characters', async () => {
    ...
    expect(
      screen.getAllByText('1').some((el) => el.closest('.bg-blue-100'))
    ).toBe(true);
    expect(
      screen.getAllByText('!').some((el) => el.closest('.bg-blue-100'))
    ).toBe(true);
  });
  ...
  it('should copy NATO alphabet to clipboard', async () => {
    ...
    const copyButton = screen.getByRole('button', {
      name: /copy nato alphabet to clipboard/i,
    });
    await user.click(copyButton);

    expect(mockWriteText).toHaveBeenCalledWith('Alpha Bravo Charlie');
  });
